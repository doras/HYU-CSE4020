...
def exp(rv):
    theta = np.sqrt(np.sum(rv ** 2))
    rv = rv / theta

    R = np.array([[  np.cos(theta) + (rv[0] ** 2) * (1 - np.cos(theta)), 
                     rv[0] * rv[1] * (1 - np.cos(theta)) - rv[2] * np.sin(theta), 
                     rv[0] * rv[2] * (1 - np.cos(theta)) + rv[1] * np.sin(theta)  ],
                  [  rv[1] * rv[0] * (1 - np.cos(theta)) + rv[2] * np.sin(theta), 
                     np.cos(theta) + (rv[1] ** 2) * (1 - np.cos(theta)), 
                     rv[1] * rv[2] * (1 - np.cos(theta)) - rv[0] * np.sin(theta)  ],
                  [  rv[2] * rv[0] * (1 - np.cos(theta)) - rv[1] * np.sin(theta), 
                     rv[2] * rv[1] * (1 - np.cos(theta)) + rv[0] * np.sin(theta), 
                     np.cos(theta) + (rv[2] ** 2) * (1 - np.cos(theta))           ]])

    return R

def log(R):
    theta = np.arccos((R[0][0] + R[1][1] + R[2][2] - 1) / 2)
    v = np.array([(R[2][1] - R[1][2]) / (2 * np.sin(theta)), 
